...
function BodyOnResize()
{
	...
	if ( !parent.frames["toolbar"].document.getElementById( "AccordionTitle" ) ||
		 !frames["index"].document.getElementById( "CatapultIndex" ) ||
    ...
    parent.frames["toolbar"].document.getElementById( "AccordionTitle" ).style.width = Math.max( FMCGetClientWidth( window, true ), 0 ) + "px";
    SetIFrameHeight();
}
...
function SetupAccordion()
{
    for ( var i = 0; i < gAccordionItems.length; i++ )
    {
        ...
        {
			accordionItem.onmouseover = function() { this.getElementsByTagName( "td" )[0].style.backgroundImage = "url('" + FMCGetMCAttribute( this, "MadCap:overImage" ) + "')"; };
			accordionItem.onmouseout = function() { this.getElementsByTagName( "td" )[0].style.backgroundImage = "url('" + FMCGetMCAttribute( this, "MadCap:outImage" ) + "')"; };
...
function SetupMouseEffectDefaults()
{
    ...
    for ( var i = 0; i < gAccordionItems.length; i++ )
    {
		...
		accordionItem.getElementsByTagName( "td" )[0].style.backgroundImage = "url('" + "Images/" + name + "Background.jpg" + "')";
...
function LoadAccordionIconsStyle( properties )
{
	...
	for ( var j = 0; j < properties.length; j++ )
	{
        ...
        {
			accordionIcons.getElementsByTagName( "td" )[0].style.backgroundImage = "url('" + parent.gRootFolder + parent.gSkinFolder + "AccordionIconsBackground.jpg')";
        }
        ...
        for ( var j = 0; j < properties.length; j++ )
        {
            ...
            else if ( cssName == "icon" )
            {
                ...
                else
                {
					...
					var width			= resourceManager.GetResourceProperty( cssValue, "Width", "auto" );
					var height			= resourceManager.GetResourceProperty( cssValue, "Height", "auto" );
            ...
            else if ( cssName == "backgroundGradient" )
            {
				...
				accordionItem.getElementsByTagName( "td" )[0].style.backgroundImage = "url('" + parent.gRootFolder + parent.gSkinFolder + name + "Background.jpg')";
        ...
        else if ( styleName == "SearchFavoritesDeleteButton" )
        {
			...
			for ( var j = 0; j < properties.length; j++ )
			{
				...
				else if ( cssName == "Icon" )
				{
					...
					var resourceManager	= FMCGetRootFrame().gResourceManager;
					var width			= resourceManager.GetResourceProperty( cssValue, "Width", null );
					var height			= resourceManager.GetResourceProperty( cssValue, "Height", null );
        ...
        else if ( styleName == "SearchFiltersLabel" )
        {
			...
			for ( var j = 0; j < properties.length; j++ )
			{
				...
				else
				{
					...
					searchFrame.gFiltersLabelStyleMap.SetItem( cssName, cssValue );
				}
			}
        }
...
function SetActiveIFrame( id, title )
{
	...
	if ( gInit )
	{
		parent.frames["toolbar"].document.getElementById( "AccordionTitle" ).firstChild.nodeValue = title;
	}
    
    gActiveItem.getElementsByTagName( "td" )[0].style.backgroundImage = "url('" + FMCGetMCAttribute( gActiveItem, "MadCap:outImage" ) + "')";
    gActiveItem.onmouseout = function () { this.getElementsByTagName( "td" )[0].style.backgroundImage = "url('" + FMCGetMCAttribute( this, "MadCap:outImage" ) + "')"; };
    ...
    gActiveItem.getElementsByTagName( "td" )[0].style.backgroundImage = "url('" + FMCGetMCAttribute( gActiveItem, "MadCap:overImage" ) + "')";
...
function SetIFrameHeight()
{
    ...
    if ( gActiveItem )
    {
		...
		var iframe	= frames[gIFrames[itemID].id];
		...
		if ( !iframe.gInit )
		{
			...
			{
				case "toc":
					gFadeElement = iframe.document.getElementsByTagName( "div" )[1];
					break;
				case "search":
			...
			StartLoading( iframe, parentElement, parent.gLoadingLabel, parent.gLoadingAlternateText, gFadeElement );
...
function SetIFrameHeight2()
{
	...
	{
		EndLoading( iframe, gFadeElement );
	}
