...
function CMCParser( str )
{
    ...
    this.ParseExpression    = function ()
    {
        ...
        else if ( Peek() == CMCToken.Word || Peek() == CMCToken.Phrase )
        {
    ...
    this.Evaluate   = function ( buildWordMap, buildPhraseMap )
    {
        ...
        if ( tokenType == CMCToken.Word )
        {
            ...
            for ( var i = 0; i < gSearchDBs.length; i++ )
            {
				...
				if ( searchDB.SearchType == "NGram" )
				{
					for ( var j = 0; j < tokenText.length - searchDB.NGramSize + 1; j++ )
					{
						var subText	= tokenText.substring( j, j + searchDB.NGramSize );
				...
				{
					searchDB.LookupStem( resultSet, tokenText, i, buildWordMap, buildPhraseMap );
				}
                ...
                for ( var j = 0; j < resultSet.GetLength(); j++ )
                {
                    ...
                    if ( result.ParentPhraseName == mToken.GetTokenText() )
                    {
                        result.Ranking = result.Ranking + 1000;
                    }
                    else if ( result.ParentPhraseName.toLowerCase() == mToken.GetTokenText().toLowerCase() )
                    {
        ...
        {
			var terms		= SplitPhrase( mToken.GetTokenText() );
            var resultSet	= null;
            ...
            for ( var i = 0; i < terms.length; i++ )
            {
                ...
                for ( var j = 0; j < gSearchDBs.length; j++ )
                {
					...
					{
						searchDB.LookupStem( resultSet2, term, j, true, buildPhraseMap );
					}
                }
...
CMCQueryResultSet.prototype.Add	= function( result, buildWordMap, buildPhraseMap, buildTopicMap )
{
    ...
    if ( buildWordMap )
    {
		...
		this.mWordMap.SetItem( key, result );
    }
    ...
    if ( buildPhraseMap )
    {
		...
		this.mPhraseMap.SetItem( key, true );
    }
    ...
    if ( buildTopicMap )
    {
		...
		{
			indexList = new Array();
			this.mTopicMap.SetItem( key, indexList );
		}
...
CMCQueryResultSet.prototype.RemoveTopicId	= function( result )
{
    ...
    if ( indexList )
    {
		for ( var i = 0; i < indexList.length; i++ )
		{
			...
			this.mWordMap.RemoveItem( wordKey );
			this.mPhraseMap.RemoveItem( phraseKey );
			this.mTopicMap.RemoveItem( topicKey );
...
CMCQueryResultSet.prototype.FindAdjacentEntries	= function( results, token, buildWordMap, buildPhraseMap )
{
    ...
    {
        wordListMap.SetItem( wordList[j], true );
    }
