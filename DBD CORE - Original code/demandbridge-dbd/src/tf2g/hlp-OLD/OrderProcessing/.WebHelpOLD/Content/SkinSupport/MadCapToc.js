...
function SetSelection( aNode )
{
    ...
    gCurrSelection.style.backgroundColor = "#224386";
}
...
function BuildToc( xmlNode, htmlNode, indent, fullPath )
{
    for ( var i = 0; i < xmlNode.childNodes.length; i++ )
    {
        ...
        if ( !aCached )
        {
            ...
            gClassToANodeMap.SetItem( className, aCached );
        }
        ...
        if ( isBook )
        {
            ...
            else
            {
				...
				if ( bookIcon )
				{
					...
					width = resourceManager.GetResourceProperty( bookIcon, "Width", 16 );
					height = resourceManager.GetResourceProperty( bookIcon, "Height", 16 );
				}
        ...
        else
        {
            ...
            else
            {
				...
				if ( topicIcon )
				{
					...
					width = resourceManager.GetResourceProperty( topicIcon, "Width", 16 );
					height = resourceManager.GetResourceProperty( topicIcon, "Height", 16 );
				}
    ...
    if ( tocEntryStyle )
    {
        ...
        if ( properties.length > 0 )
        {
            ...
            for ( var i = 0; i < props.length; i++ )
            {
                ...
                nameToValueMap.SetItem( prop.getAttribute( "Name" ), FMCGetPropertyValue( prop, null ) );
            }
            
            gStylesMap.SetItem( "TocEntry_" + tocEntryStyle.getAttribute( "Name" ), nameToValueMap );
        }
        ...
        for ( var i = 0; i < styleClasses.length; i++ )
        {
            ...
            if ( properties.length > 0 )
            {
                ...
                gStylesMap.SetItem( "TocEntry_" + styleClasses[i].getAttribute( "Name" ), nameToValueMap );
            }
        }
    }
}
...
function CreateToc( a )
{
    ...
    var xmlDoc	= CMCXmlParser.GetXmlDoc( xmlPath, true, OnTocXmlLoaded, a );
}
