...
function LoadChunk( index )
{
	...
	else
	{
		...
		xmlDoc = CMCXmlParser.GetXmlDoc( parent.parent.gRootFolder + "Data/" + chunk.getAttribute( "Link" ), false, null, null );
...
function IndexEntryOnmouseout()
{
	...
	this.style.color = color ? color : "#AAC2CE";
}
...
function SetLinkMap( term, indexLinks )
{
    ...
    for ( var i = 0; i < indexLinks.length; i++ )
    {
        ...
        linkMap.SetItem( indexLink.Title, indexLink.Link );
    }
    ...
    if ( chunks.length == 0 )
    {
        ...
        for ( var i = 0; i < xmlNode.childNodes.length; i++ )
        {
            ...
            if ( entry.nodeName == "IndexEntry" )
            {
                ...
                {
                    gAlphaMap.SetItem( first, gIndexEntryCount );
                }
...
function Init()
{
	...
	gXMLDoc = parent.parent.GetMasterHelpSystem().GetIndex();
...
function HighlightEntry( node )
{
    if ( gSelectedItem )
    {
        ...
        gSelectedItem.firstChild.style.color = color ? color : "#AAC2CE";
