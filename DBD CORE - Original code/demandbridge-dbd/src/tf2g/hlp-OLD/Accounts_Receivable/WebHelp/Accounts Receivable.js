...
var gHelpSystemName = "Accounts Receivable.htm";
...
function FMCOpenHelp2( webHelpFile, id, skinName, searchQuery, firstPick )
{
        ...
        else
        {
            if ( xmlDoc )
            {
                ...
                for ( var i = 0; i < maps.length; i++ )
                {
                    ...
                    {
                        gCSHData.topic = maps[i].getAttribute( "Link" );
                        gCSHData.skinName = maps[i].getAttribute( "Skin" );
                        
                        if ( gCSHData.skinName )
                        {
                            gCSHData.skinName = maps[i].getAttribute( "Skin" ).substring( "Skin".length, gCSHData.skinName.indexOf( "/" ) );
                        }
    ...
    if ( skinName )
    {
        gCSHData.skinName = skinName;
    }
    else if ( !gCSHData.skinName )
    {
        if ( xmlDoc )
        {
            gCSHData.skinName = xmlDoc.documentElement.getAttribute( "DefaultSkinName" );
        }
        else
        {
    ...
    if ( gCSHData.skinName )
    {
        xmlDoc = CMCXmlParser.GetXmlDoc( webHelpPath + "Data/Skin" + gCSHData.skinName + "/Skin.xml", false, null, null );
        
        if ( xmlDoc )
        {
			var xmlHead		= xmlDoc.getElementsByTagName( "CatapultSkin" )[0];
			var useDefault	= xmlHead.getAttribute( "UseDefaultBrowserSetup" ) == "True";
